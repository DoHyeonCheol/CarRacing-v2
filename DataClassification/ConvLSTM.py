...
from keras.layers import ConvLSTM2D, Dense, Dropout, Flatten
...
def load_file(filepath, window_size=8):
    ...
    for data_point in data_points:
        ...
        if len(episode) == 2 and episode == [0.0, 0.0]:
            episodes.append(episode + [0.0] * (window_size - len(episode)))
            episode = []
        elif len(episode) == window_size:
...
def load_dataset(prefix='C:/Users/Dohc/PycharmProjects/test/Real/', window_size=8):
    ...
    # load sequences
    dqn_data = load_dataset_group([prefix + 'edit/DQN.txt'], window_size)
    ppo_data = load_dataset_group([prefix + 'edit/PPO.txt'], window_size)
    keyboard_data = load_dataset_group([prefix + 'train_keyboard.txt'], window_size)
...
def evaluate_and_predict_model(trainX, trainy, testX, testy, classificationX, classificationy, window_size=8, experiment_name='test_ConvLSTM_1'):
    verbose, epochs, batch_size = 1, 300, 32
    n_features, n_outputs = trainX.shape[2], trainy.shape[1]

    # reshape data into time steps of sub-sequences
    n_steps, n_length = 4, 2
    trainX = trainX.reshape((trainX.shape[0], n_steps, n_length, n_features, 1))
    testX = testX.reshape((testX.shape[0], n_steps, n_length, n_features, 1))
    classificationX = classificationX.reshape((classificationX.shape[0], n_steps, n_length, n_features, 1))

    model = Sequential()
    model.add(ConvLSTM2D(filters=64, kernel_size=(1, 1), activation='relu', input_shape=(n_steps, n_length, n_features, 1)))
...
# run experiment
def run_experiment(window_size=8, experiment_name='test_ConvLSTM_1'):
