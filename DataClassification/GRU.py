...
# 모델 평가 및 예측 함수
def evaluate_and_predict_model(trainX, trainy, testX, testy, classificationX, classificationy, window_size=8,
                               experiment_name='test_GRU_1'):
    verbose, epochs, batch_size = 1, 300, 32
    n_features, n_outputs = trainX.shape[2], trainy.shape[1]

    model = Sequential()
    model.add(GRU(64, input_shape=(window_size, n_features)))
...
# run experiment
def run_experiment(window_size=8, experiment_name='test_GRU_1'):
    ...
    accuracy, predictions = evaluate_and_predict_model(trainX, trainy, testX, testy, classificationX, classificationy,
                                                       window_size, experiment_name)
