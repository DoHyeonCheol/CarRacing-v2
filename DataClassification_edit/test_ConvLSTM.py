...
def build_convlstm_model(input_shape):
    ...
    # ConvLSTM2D는 5D 입력을 기대하므로 reshape 필요
    x = layers.Reshape((input_shape[0], 1, input_shape[1], 1))(inputs)
    x = layers.ConvLSTM2D(filters=64, kernel_size=(3, 1), padding='same', return_sequences=True)(x)
    x = layers.BatchNormalization()(x)
    x = layers.ConvLSTM2D(filters=32, kernel_size=(3, 1), padding='same', return_sequences=False)(x)
    x = layers.BatchNormalization()(x)
    x = layers.Flatten()(x)
